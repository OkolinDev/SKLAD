...
class Tree(tk.Toplevel):
    ...
    def tree(self):
        self.tree1 = ttk.Treeview(self, columns=('ID', 'name', 'model', 'serial', 'quantity', 'shelf', 'place'),
                                  height=15, show="headings")

        self.tree1.column('ID', width=30, anchor=tk.CENTER)
        self.tree1.column('name', width=90, anchor=tk.CENTER)
...
class Chek(tk.Toplevel):
    ...
    def check_in(self):
        ...
        namecheck = self.entryCheck, self.entrySerial
        checkrow = self.db.c.execute("SELECT name, serial from sklad")
        print(checkrow)
        ...
        # else:
        # self.tr.view_record_db()
        # self.db.c.execute("SELECT id, name, model, serial, quantity, shelf, place  from sklad WHERE name, serial = self.entryCheck, self.entrySerial")
        # self.check = self.db.c.fetchall()
...
class DB:
    def __init__(self):
        ...
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS sklad (id int primary key, name text, model text, serial text, 
            quantity int, shelf INT, place INT) ''')
        self.conn.commit()
